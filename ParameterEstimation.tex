...
A mixture of $n$ Gaussian models is characterized by the density
\begin{equation}
...
\end{equation}
where $\bm p=(p_1,p_2, \dots, p_n)$ is the vector of probabilities,
$\bm \mu = (\mu_1, \mu_2,\dots, \mu_n)$ is the vector of means,
and
$\bm \sigma^2 = (\sigma^2_1, \sigma^2_2,\dots, \sigma^2_n)$ is the vector of variances.
In this way, the parameter space is $\bm \Theta =
\mathcal S^n\times \mathbbm R^n \times \mathbbm R_+^n$, where
$\mathcal S^n$ is the surface of the $n$-dimensional simplex.
This space is a subset of $\mathbbm R^{(n-1)n^2}$. 
...
$\operatorname{E}X = \sum_{j=1}^{n} p_j\mu_j$ and that
$\operatorname{Var}X = \sum_{j=1}^{n} p_j\sigma^2_j$.
...
\subsection{The $\mathcal{G}_I^0$ distribution}

Assuming that the backscatter is $X\sim\Gamma^{-1}(\alpha,\gamma)$,
the speckle is $Y\sim\Gamma(1,L)$, and that they are independent random variables, we have that the return follows a $\mathcal{G}_I^0$ distribution\cite{GeodesicDistanceGI0JSTARS}%
...
But we can also set $k=2$ and obtain a second-order moment estimator, using $\operatorname{E}U^2=\frac{1}{3}\theta^2$ it is immediate that
...
We need $(n-1)n^2$ linearly independent equations to estimate $\bm{\theta} = (\bm p, \bm \mu, \bm \sigma^2)$.
...
This is the well-known estimator for the number of looks (equivalent number of looks) which consists of the reciprocal of the squared coefficient of variation.
...
Since we assume no knowledge about the number of components $n$, it is important to use a balance between the number of parameters and the likelihood, otherwise we may end with a model with the largest possible $n$.
...
$$[it seems strange that the equation of AIC is given here, while BIC is adopted in the following codes.]
...
\begin{equation}
\ell(\alpha,\gamma;\widehat L, \bm Z) =\log
\frac{\Gamma(\widehat L-\alpha)}{\gamma^\alpha \Gamma(-\alpha)} +
\widehat L \sum_{i=1}^n \log\frac{Z_i}{\gamma+\widehat L Z_i} + 
\alpha \sum_{i=1}^n \log(\gamma + \widehat L Z_i),
